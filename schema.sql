...
CREATE INDEX IF NOT EXISTS idx_ecu_chat_channels_updated ON ecu_chat_channels(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_ecu_chat_channels_file ON ecu_chat_channels(file_sha256, updated_at DESC);

CREATE TABLE IF NOT EXISTS ecu_chat_messages (
  id TEXT PRIMARY KEY,
  ...
  provider TEXT,
  created_at INTEGER NOT NULL,
  FOREIGN KEY(channel_id) REFERENCES ecu_chat_channels(id) ON DELETE CASCADE
);
