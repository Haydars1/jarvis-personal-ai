...
async function testCredential(env,c){
  ...
  try{
    if(p==='gemini'){const model=c.model||'gemini-2.5-flash';const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}?key=${encodeURIComponent(sec)}`);if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(p==='groq'){const r=await fetch('https://api.groq.com/openai/v1/models',{headers:{authorization:'Bearer '+sec}});if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(p==='openrouter'){const r=await fetch('https://openrouter.ai/api/v1/models',{headers:{authorization:'Bearer '+sec}});if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(p==='nvidia'){const r=await fetch('https://integrate.api.nvidia.com/v1/models',{headers:{authorization:'Bearer '+sec}});if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(p==='google'){if(!String(c.endpoint||'').includes('.apps.googleusercontent.com'))throw Error('GOOGLE_CLIENT_ID_INVALID')}
    else if(p==='meta'){const r=await fetch('https://graph.facebook.com/v24.0/me?access_token='+encodeURIComponent(sec));if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(p==='github'){const r=await fetch('https://api.github.com/user',{headers:{authorization:'Bearer '+sec,accept:'application/vnd.github+json','user-agent':'JARVIS-SelfUpdate'}});if(!r.ok)throw Error('HTTP_'+r.status)}
    else if(c.endpoint){const h={'authorization':'Bearer '+sec};const r=await fetch(c.endpoint.replace(/\/$/,'')+'/models',{headers:h});if(!r.ok&&r.status!==404)throw Error('HTTP_'+r.status)}
...
async function gh(c,path,opts={}){
  const r=await fetch('https://api.github.com'+path,{...opts,headers:{accept:'application/vnd.github+json','x-github-api-version':'2022-11-28','user-agent':'JARVIS-SelfUpdate',authorization:'Bearer '+c.secret,...(opts.headers||{})}});
...
async function rememberExplicit(env,text){
  ...
  if(!/(bunu|şunu|sunu)?.*(hatırla|hafızana|belleğine|bellegine|kaydet)/.test(l))return null;
  const clean=String(text).replace(/^(jarvis[, ]*)?/i,'').replace(/(bunu|şunu|sunu)?\s*(hatırla|hafızana|belleğine|bellegine)\s*(kaydet)?[: ]*/i,'').trim();
...
async function ddg(q){const r=await fetch('https://html.duckduckgo.com/html/?q='+encodeURIComponent(q),{headers:{'user-agent':'Mozilla/5.0 JARVIS/4.0'}});if(!r.ok)throw Error('SEARCH_'+r.status);const h=await r.text(),out=[],re=/<a[^>]+class="result__a"[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>[\s\S]*?<a[^>]+class="result__snippet"[^>]*>([\s\S]*?)<\/a>/g;const clean=s=>s.replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').replace(/&#x27;/g,"'").replace(/\s+/g,' ').trim();let m;while((m=re.exec(h))&&out.length<8)out.push({title:clean(m[2]),url:m[1].replace(/&amp;/g,'&'),snippet:clean(m[3])});return out}
async function aiFallback(env,messages){const errs=[];
 const vault=await credentialSecrets(env,'chat');for(const c of vault){try{const text=await callVaultAI(env,c,messages);await run(env,'UPDATE credentials SET last_status=?,last_error=NULL,last_test_at=?,updated_at=? WHERE id=?','ok',now(),now(),c.id);return{provider:c.label||c.provider,text}}catch(e){errs.push((c.label||c.provider)+':'+e.message);await run(env,'UPDATE credentials SET last_status=?,last_error=?,last_test_at=?,updated_at=? WHERE id=?','error',e.message,now(),now(),c.id)}}
 if(env.GEMINI_API_KEY)try{const model=env.GEMINI_MODEL||'gemini-2.5-flash';const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${env.GEMINI_API_KEY}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({contents:messages.filter(x=>x.role!=='system').map(x=>({role:x.role==='assistant'?'model':'user',parts:[{text:x.content}]})),systemInstruction:{parts:[{text:messages.find(x=>x.role==='system')?.content||''}]}})});if(!r.ok)throw Error('GEMINI_'+r.status);return{provider:'Gemini',text:(await r.json()).candidates?.[0]?.content?.parts?.map(p=>p.text).join('')||''}}catch(e){errs.push(e.message)}
 if(env.GROQ_API_KEY)try{const r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+env.GROQ_API_KEY},body:JSON.stringify({model:env.GROQ_MODEL||'llama-3.3-70b-versatile',messages,temperature:.25})});if(!r.ok)throw Error('GROQ_'+r.status);return{provider:'Groq',text:(await r.json()).choices?.[0]?.message?.content||''}}catch(e){errs.push(e.message)}
 if(env.OPENROUTER_API_KEY)try{const r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+env.OPENROUTER_API_KEY,'HTTP-Referer':'https://jarvis.local','X-Title':'JARVIS'},body:JSON.stringify({model:env.OPENROUTER_MODEL||'openrouter/auto',messages,temperature:.25})});if(!r.ok)throw Error('OPENROUTER_'+r.status);return{provider:'OpenRouter',text:(await r.json()).choices?.[0]?.message?.content||''}}catch(e){errs.push(e.message)}
 if(env.AI){
  ...
  for(const model of cfModels){
   ...
   try{
    const out=await env.AI.run(model,{messages,temperature:0.25,max_tokens:1200});
 ...
 throw Error(errs.length?'ALL_AI_FAILED:'+errs.join('|'):'NO_AI_PROVIDER')}
...
async function command(env,text){await log(env,'user',text);const l=text.toLocaleLowerCase('tr-TR');const remembered=await rememberExplicit(env,text);if(remembered){const reply=`Bunu hafızama kaydettim: ${remembered}`;await log(env,'jarvis',reply);return{reply,action:'memory'}}
 ...
 const s=await state(env),history=await chatHistory(env,30),mem=await qall(env,'SELECT text,tags FROM memories ORDER BY created_at DESC LIMIT 30');const system=`Sen JARVIS adlı Türkçe kişisel asistansın. ChatGPT gibi doğal sohbet et ama aynı zamanda aksiyon alan kişisel asistansın. Kullanıcının önceki sohbetlerini ve hafızasını bağlam olarak kullan. Kısa gerektiğinde kısa, detay gerektiğinde detaylı ol. Bağlı olmayan entegrasyonları uydurma. Kullanıcı işi bitirmeni ister; gerektiğinde araştır, planla ve bağlı araçlar arasında geçiş yap. Geri döndürülemez işlemlerde onay iste. Kalıcı hafıza: ${JSON.stringify(mem)} Sistem bağlamı: ${JSON.stringify({brief:s.brief,tasks:s.tasks.slice(0,20),projects:s.projects.slice(0,20),integrations:s.integrations})}`;const a=await aiFallback(env,[{role:'system',content:system},...history.slice(-20,-1).map(x=>({role:x.role==='assistant'?'assistant':'user',content:x.content})),{role:'user',content:text}]);const reply=a.text||'Yanıt üretilemedi.';await log(env,'jarvis',reply,{provider:a.provider});return{reply,action:'ai',provider:a.provider}}
async function router(req,env){const u=new URL(req.url),p=u.pathname,m=req.method;
 ...
 if(p==='/api/chat/send'&&m==='POST'){const b=await body(req),text=String(b.text||'').trim();if(!text)return j({error:'EMPTY'},400);await addChat(env,'user',text,null);try{const r=await command(env,text);await addChat(env,'assistant',r.reply,r.provider||null);return j({...r,state:await state(env),history:await chatHistory(env,120)})}catch(e){await addChat(env,'assistant','Bir hata oluştu: '+e.message,'system');throw e}}
