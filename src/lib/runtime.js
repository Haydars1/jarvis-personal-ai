...
async function credentialKey(env) {
  ...
  if (credentialKeyCache.source === source && credentialKeyCache.key) {
    return credentialKeyCache.key;
  }
  ...
  if (!bytes) {
    bytes = new Uint8Array(await crypto.subtle.digest('SHA-256', encoder.encode(source)));
  }
