...
const defaultRepository = {
  async getState(env) {
    ...
    const hypotheses = await env.DB.prepare("SELECT COUNT(*) AS count FROM ecu_change_hypotheses WHERE verification_state='UNVERIFIED'").first();
    ...
    return {
      ...
      automaticHypotheses: Number(hypotheses?.count || 0),
  ...
  return {
    ...
    async status(env) {
      ...
      return {
        ...
        automaticHypotheses: Number(state.automaticHypotheses || 0),
        minVerifiedExamples,
