...
async function callJson(core, request, env, ctx) {
  const response = await core.fetch(request, env, ctx);
  if (!response.ok) return null;
  try { return await response.json(); } catch { return null; }
}

async function research(core, req, env, ctx, text) {
  const url = new URL('/api/google-search', req.url);
  url.searchParams.set('q', text);
  url.searchParams.set('num', '4');
  const payload = await settleWithin(
    callJson(core, new Request(url, { headers: req.headers }), env, ctx),
    LIMITS.researchMs,
    null
  );
...
async function tools(core, req, env, ctx, text) {
...
async function orchestratedChat(core, req, env, ctx, text) {
  ...
  let payload;
  try { payload = await baseResponse.clone().json(); } catch { return baseResponse; }
  ...
  payload.trace = [
    ...(Array.isArray(payload.trace) ? payload.trace : []),
    ...(experts.picks.length ? [{ kind: 'pool', label: 'JARVIS uzman havuzu', value: `${experts.kind}: ${experts.picks.map(item => `${item.provider} (${item.score})`).join(' + ')}` }] : []),
...
export function createChatOrchestrator(core) {
  if (!core?.fetch) throw new Error('CHAT_CORE_FETCH_REQUIRED');
  return async function handleChat(req, env, ctx) {
    let text = '';
    try { text = String((await req.clone().json())?.text || '').trim(); } catch {}
    if (!text) return core.fetch(req, env, ctx);
    return needsOrchestration(text)
      ? orchestratedChat(core, req, env, ctx, text)
      : simpleChat(core, req, env, ctx, text);
  };
}
