...
async function searchGoogle(env,q,{num=10,start=1,searchType=''}={}){const c=await cfg(env);if(!c)throw Error('GOOGLE_SEARCH_NOT_CONFIGURED');const u=new URL('https://customsearch.googleapis.com/customsearch/v1');u.searchParams.set('key',c.key);u.searchParams.set('cx',c.cx);u.searchParams.set('q',String(q||''));u.searchParams.set('num',String(Math.min(10,Math.max(1,num))));u.searchParams.set('start',String(Math.max(1,start)));if(searchType==='image')u.searchParams.set('searchType','image');const r=await fetch(u,{headers:{accept:'application/json'}});if(!r.ok){const t=await r.text().catch(()=> '');throw Error('GOOGLE_SEARCH_'+r.status+(t?':'+t.slice(0,240):''))}const x=await r.json();return (x.items||[]).map(i=>({title:i.title||'',url:i.link||'',snippet:i.snippet||'',displayLink:i.displayLink||'',source:'Google',type:searchType||'web',thumbnail:i.image?.thumbnailLink||null,contextUrl:i.image?.contextLink||null,mime:i.mime||null}))}
async function apiSearch(req,env){const u=new URL(req.url),q=String(u.searchParams.get('q')||'').trim();if(!q)return json({error:'QUERY_REQUIRED'},400);const type=String(u.searchParams.get('type')||'').toLowerCase()==='image'?'image':'';const results=await searchGoogle(env,q,{num:Number(u.searchParams.get('num')||10),searchType:type});return json({provider:'Google',type:type||'web',results})}
