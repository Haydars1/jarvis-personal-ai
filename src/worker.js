...
async function router(req,env,ctx=null){const u=new URL(req.url),p=u.pathname,m=req.method;
 ...
 if(p==='/api/chat/send'&&m==='POST'){const b=await body(req),text=String(b.text||'').trim(),attachmentContext=String(b.attachmentContext||'').trim(),analysisText=[text,attachmentContext].filter(Boolean).join('\n\n'),directAttachmentReply=String(b.attachmentDirectReply||'').trim();if(!text&&!attachmentContext)return j({error:'EMPTY'},400);const ts=now();try{if(directAttachmentReply){const history=[{role:'user',content:text||'Dosya gönderildi',provider:null,created_at:ts},{role:'assistant',content:directAttachmentReply,provider:'JARVIS File Analyzer',created_at:now()}];const persist=async()=>{try{await addChat(env,'user',text||'Dosya gönderildi',null);await addChat(env,'assistant',directAttachmentReply,'JARVIS File Analyzer');await log(env,'file-analysis',directAttachmentReply,{attachments:b.attachmentMeta||[]})}catch(e){await recordRuntimeError(env,e,'chat.attachment.persist')}};if(ctx?.waitUntil)ctx.waitUntil(persist());else persist();return j({reply:directAttachmentReply,action:'attachment_analysis',provider:'JARVIS File Analyzer',history})}if(/ai.*(durum|test|kontrol|diag)|sağlayıcı.*(durum|test|kontrol)|saglayici.*(durum|test|kontrol)/i.test(text)){const d=await aiDiagnostics(env);const reply=d.ok?('AI sistemi çalışıyor. Çalışan sağlayıcı sayısı: '+d.working+'/'+d.total+'. '+d.recommendation):('AI sistemi çalışmıyor. '+d.recommendation);return j({reply,action:'ai_diagnostics',provider:'JARVIS Diagnostics',diagnostics:d,history:[{role:'user',content:text,provider:null,created_at:ts},{role:'assistant',content:reply,provider:'JARVIS Diagnostics',created_at:now()}]})}const r=await withTimeout(quickCommand(env,analysisText||text),9500,'QUICK_COMMAND_TIMEOUT');const history=[{role:'user',content:text||'Dosya gönderildi',provider:null,created_at:ts},{role:'assistant',content:r.reply,provider:r.provider||null,created_at:now()}];const persist=async()=>{try{await addChat(env,'user',text||'Dosya gönderildi',null);await addChat(env,'assistant',r.reply,r.provider||null);await log(env,'jarvis-fast',r.reply,{provider:r.provider,action:r.action,attachments:b.attachmentMeta||[]})}catch(e){await recordRuntimeError(env,e,'chat.persist')}};if(ctx?.waitUntil)ctx.waitUntil(persist());else persist();return j({...r,history})}catch(e){const msg=String(e?.message||e||'UNKNOWN'),reply=localFallbackAnswer(analysisText||text,msg)||researchFallbackAnswer(analysisText||text,[])||'Cevap motoru zamanında dönemedi. İsteğini kaydettim; teknik hata detayını sana dökmüyorum.';const history=[{role:'user',content:text||'Dosya gönderildi',provider:null,created_at:ts},{role:'assistant',content:reply,provider:'JARVIS Local',created_at:now()}];const persist=async()=>{try{await recordRuntimeError(env,e,'chat.send');await addChat(env,'user',text||'Dosya gönderildi',null);await addChat(env,'assistant',reply,'JARVIS Local')}catch{}};if(ctx?.waitUntil)ctx.waitUntil(persist());else persist();return j({reply,action:'local_fallback',provider:'JARVIS Local',history})}}
